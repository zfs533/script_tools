...
resign() {
    ...
    if [ "$APP_PATH" = "$TEMP_DIR/Payload/Hiha.app" ]; then
        python3 "$(dirname $0)/mobileprovision2plist.py" "$TEMP_DIR/newEntitlements.plist" "icloud"
    fi
    
    # 自动输入权限密码，不同弹出框手动输入
    security set-key-partition-list -S apple-tool:,apple: -s -k "Tibao11." "$HOME/Library/Keychains/login.keychain-db"  >/dev/null 2>&1
    # 真正的签名
    echo "$CERTIFICATE"
    /usr/bin/codesign -f -s "$CERTIFICATE" --entitlements "$TEMP_DIR/newEntitlements.plist" "$APP_PATH" >/dev/null 2>&1
    checkStatus
...
# 遍历所有需要重签名的组件
resignPaths=$(find "$TEMP_DIR/Payload/$APP_NAME" -d -name *.app -o -name *.framework -o -name *.dylib -o -name *.appex)
